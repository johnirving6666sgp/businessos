...
const SOURCE_LABELS = { conversation: '对话', opportunity: '商机', broadcast: '广播', proposal: '方案', manual: '手动' }
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-white flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800">任务</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" pill onClick={() => setShowAdd(true)} icon={<Icon name="plus" size={15} />}>创建</Button>
        </div>
      </div>
      ...
      <div className="flex gap-1.5 px-4 py-2 bg-white border-b border-slate-50 overflow-x-auto">
        <button
          ...
          className={`text-xs px-3 py-1 rounded-full flex-shrink-0 transition-colors ${!filterStatus ? 'bg-blue-500 text-white' : 'text-slate-500 hover:bg-slate-100'}`}
        >
          全部
        </button>
        {STATUSES.map(s => (
          <button
            ...
            className={`text-xs px-3 py-1 rounded-full flex-shrink-0 transition-colors ${filterStatus === s ? 'bg-blue-500 text-white' : 'text-slate-500 hover:bg-slate-100'}`}
          >
            {STATUS_LABELS[s]}
          </button>
        ))}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          {tasks.length === 0 && !loading && (
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-10 flex flex-col items-center text-center">
              ...
              <p className="text-sm font-medium text-slate-700">{filterMine ? '你没有待办任务' : '暂无任务'}</p>
              <p className="text-xs text-slate-400 mt-1">清爽，享受这一刻</p>
            </div>
          )}
        </div>
      )}
...
function TaskCard({ task, onStatusChange }) {
  ...
  return (
    <div className={`bg-white border rounded-2xl overflow-hidden transition-all ${isOverdue ? 'border-red-200' : 'border-slate-100'} shadow-sm hover:shadow`}>
      ...
      {expanded && (
        <div className="px-4 pb-3 border-t border-slate-50 pt-2 space-y-2">
          {task.description && (
            <p className="text-sm text-slate-600 bg-slate-50 rounded-xl p-2">{task.description}</p>
          )}
          {nextStatuses.length > 0 && (
            <div className="flex gap-2">
              {nextStatuses.map(s => (
                <Button key={s} size="sm" variant="secondary" pill onClick={() => onStatusChange(task.id, s)}>
                  → {STATUS_LABELS[s]}
                </Button>
              ))}
            </div>
    ...
    <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-md p-5 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <input value={form.title} onChange={e => update('title', e.target.value)} placeholder="任务标题 *" className={INPUT_CLASS} required autoFocus />
          <textarea value={form.description} onChange={e => update('description', e.target.value)} placeholder="详细描述（选填）" rows={2} className={INPUT_CLASS + ' resize-none'} />
          ...
          <div className="flex gap-2 pt-1">
            ...
            <Button type="submit" pill loading={loading} className="flex-1">创建</Button>
          </div>
        </form>
      </div>
    </div>

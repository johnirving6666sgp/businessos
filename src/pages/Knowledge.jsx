...
export function Knowledge({ onNavigate }) {
  ...
  return (
    <div className="flex h-full">
      ...
      <div className={`flex flex-col ${selected ? 'w-80 flex-shrink-0 border-r border-slate-100' : 'flex-1'}`}>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-white flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-400">{total} 条知识</p>
          </div>
        ...
        <div className="px-4 py-2 border-b border-slate-100 bg-white flex-shrink-0">
          <input
            ...
            placeholder="搜索标题或内容..."
            className="w-full border border-slate-200 rounded-xl px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300"
          />
        </div>
          ...
          <div className="flex items-center gap-1.5 px-4 py-2 border-b border-slate-100 bg-white flex-shrink-0 overflow-x-auto">
            <button
              ...
              className={`text-xs px-2.5 py-1 rounded-full whitespace-nowrap transition-colors flex-shrink-0 ${
                !tagFilter ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
              }`}
            >
              全部
            </button>
            {allTags.map(tag => (
              <button
                ...
                className={`text-xs px-2.5 py-1 rounded-full whitespace-nowrap transition-colors flex-shrink-0 ${
                  tagFilter === tag ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                }`}
              >
                #{tag}
              </button>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!loading && items.length === 0 && (
            <div className="text-center py-16 text-slate-400">
              ...
              <p>{search ? `未找到"${search}"` : '暂无知识条目'}</p>
              <p className="text-sm mt-1">{search ? '换个关键词试试' : '点击"新建"开始沉淀知识'}</p>
            </div>
          )}
            ...
            {items.map(item => (
              ...
              >
                ...
                {item.content_preview && (
                  <p className="text-xs text-slate-500 leading-relaxed line-clamp-2">{item.content_preview}</p>
                )}
                {item.tags?.length > 0 && (
                ...
                <p className="text-xs text-slate-300 mt-1.5">
                  {item.author_name} · {new Date(item.updated_at).toLocaleDateString('zh-CN')}
                </p>
              </button>
            ))}
          </div>
      ...
      {/* 新建弹窗 */}
      {showCreate && (
        <CreateKnowledgeModal onClose={() => setShowCreate(false)} onCreate={handleCreate} />
      )}
    </div>
  )
}
      ...
      <div className="flex items-start justify-between px-5 py-3 border-b border-slate-100 flex-shrink-0 bg-white gap-3">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <Button
            ...
            onClick={() => {
              onNavigate?.('chat')
              toast('已跳转到AI对话，可继续深化这条知识', 'success')
            }}
          >
            深化讨论
          </Button>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        ...
        {data.source_type && (
          <p className="text-xs text-slate-400 mt-4">
            来源：{SOURCE_LABELS[data.source_type] || data.source_type}
          </p>
        )}
      </div>
    </div>
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          <Field label="标题 *">
            <input
              ...
              placeholder="如：VIM 真空感应熔炼炉常见故障处理手册"
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button type="submit" loading={saving} disabled={!form.title.trim() || !form.content.trim()}>保存</Button>
          </div>
        </form>
      </div>
    </div>
